  ...
  <defs>
    ...
    <div class="container">
        <div class="d-flex justify-content-center">
            <div class="card rounded">
                <img style="max-width: 450px;border-radius: 1.25rem 1.25rem 0 0 !important;" src="{{$word_id->image}}"
                     alt="">
                <div class="card-body">
                    <div class="d-flex justify-content-center">
                        @for ($i = 1; $i <= $word_id->word_length; $i++)
                            <input type="text" class="verification-input border-top-0 border-left-0 border-right-0 "
                                   maxlength="1" name="digit{{ $i }}" required>
                        @endfor
                    </div>
                    <div class="d-flex justify-content-center mt-3">
                        <button class="btn btn-success" onclick="sendVerification()">Doğrula</button>
                        <button class="btn btn-info ml-3" type="button" onclick="clearInputs()">Temizle</button>

                    </div>
                    <div class="d-flex justify-content-center mt-3">
                        <p class="card-text">{{$word_id->tips}}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function sendVerification() {
            ...
            // AJAX kullanarak isteği atıyoruz
            fetch('/checkWord', {
                method: 'POST',
                headers: {
                ...
                .then(data => {
                    ...
                    if (data.result) {
                        // Swal kullanarak doğrulama başarılı bildirimi gösteriyoruz
                        Swal.fire({
                            icon: 'success',
                            title: 'Doğrulama Başarılı!',
                            text: 'Kelime doğru.',
                            confirmButtonText: 'Sonraki Kelime',
                        }).then(() => {
                            // Sayfayı yenile
                            window.location.reload();
                        });
                    } else {
                        // Swal kullanarak doğrulama başarısız bildirimi gösteriyoruz
                        Swal.fire({
                            icon: 'error',
        ...
        inputs.forEach((input, index) => {
            input.addEventListener('input', (event) => {
                ...
                if (currentValue.length === 1) {
                    if (index < inputs.length - 1) {
                        ...
                        // Son inputa geldiyse submit butonuna odaklan
            ...
            input.addEventListener('keydown', (event) => {
                if (event.key === "Backspace") {
                    // Sil tuşuna basıldı ve input alanı boşsa bir önceki inputa odaklan
                    if (index > 0 && event.target.value === "") {
                        inputs[index - 1].focus();
                    }
                } else if (event.key === "Delete") {
                    // Delete tuşuna basıldı ve input alanı boşsa bir sonraki inputa odaklan
                    if (index < inputs.length - 1 && event.target.value === "") {
                        inputs[index + 1].focus();
                    }
                } else if (event.key === "Enter") {
                    // Enter tuşuna basıldıysa, doğrulama fonksiyonunu çağır
                    sendVerification();
                } else if (event.key === "ArrowRight") {
                    // Sağ ok tuşuna basıldıysa, bir sonraki inputa odaklan
                    if (index < inputs.length - 1) {
                        inputs[index + 1].focus();
                    }
                } else if (event.key === "ArrowLeft") {
                    // Sol ok tuşuna basıldıysa, bir önceki inputa odaklan
                    if (index > 0) {
                        inputs[index - 1].focus();
                    }
                }
